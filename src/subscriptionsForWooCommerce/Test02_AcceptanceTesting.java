...
public class Test02_AcceptanceTesting {
	...
	public static void main(String[] args) {
		...
		driver.findElement(By.xpath("//h1[normalize-space()='SUBSCRIPTIONS FOR WOOCOMMERCE']")).isDisplayed();
		System.out.println("SUBSCRIPTIONS FOR WOOCOMMERCE text is present of plugin");
		
		driver.findElement(By.xpath("//a[@class='mwb-link']")).isDisplayed();
		System.out.println("Support link is present");
		
		driver.findElement(By.id("subscriptions-for-woocommerce-overview")).isDisplayed();
		System.out.println("Overview Tab is present");
		
		driver.findElement(By.id("subscriptions-for-woocommerce-general")).isDisplayed();
		System.out.println("General Setting Tab is present");
		
		driver.findElement(By.id("subscriptions-for-woocommerce-subscriptions-table")).isDisplayed();
		System.out.println("Subscriptions Table Tab is present");
		
		driver.findElement(By.id("subscriptions-for-woocommerce-system-status")).isDisplayed();
		System.out.println("System status Tab is present");

		// Verifying General settings availability
		driver.findElement(By.id("subscriptions-for-woocommerce-general")).click();

		driver.findElement(By.xpath("//label[normalize-space()='Enable/Disable Subscription']")).isDisplayed();
		driver.findElement(By.xpath("//label[normalize-space()='Add to cart text']")).isDisplayed();
		driver.findElement(By.xpath("//label[normalize-space()='Place order text']")).isDisplayed();
		driver.findElement(By.xpath("//label[normalize-space()='Allow Customer to cancel Subscription']")).isDisplayed();
		driver.findElement(By.xpath("//label[normalize-space()='Check this box to enable the subscription.']")).isDisplayed();
		driver.findElement(By.xpath("//label[contains(text(),'Enable this option to allow the customer to cancel')]")).isDisplayed();
		driver.findElement(By.xpath("//input[@placeholder='Add to cart button text']")).isDisplayed();
		driver.findElement(By.xpath("//input[@placeholder='Place order button text']")).isDisplayed();
		driver.findElement(By.xpath("//div[contains(text(),'Use this option to change add to cart button text.')]")).isDisplayed();
		driver.findElement(By.xpath("//div[contains(text(),'Use this option to change place order button text.')]")).isDisplayed();
		...
		if(addtocart = false) {
			...
			driver.findElement(By.id("//span[@class='mdc-button__ripple']")).click();
		...
		driver.findElement(By.id("mwb_sfw_cancel_subscription_for_customer")).isDisplayed();
		if(addtocart = false) {
